...
- (void)onAppWillEnterForeground:(NSNotification *)notification {
    if ([self shouldReloadWebView]) {
        ...
        NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:@"ionic://localhost/login"]];
        //Jason added on 14Oct2020 to prevent reload to page that was blank
        [(WKWebView*)_engineWebView loadRequest:request];
        /*
         swift version
         let myURL = URL(string:"ionic://localhost/login")
...
- (BOOL)shouldReloadWebView
{
    ...
    return [self shouldReloadWebView:wkWebView.URL title:wkWebView.title];//Jason prevent webview reload
}
